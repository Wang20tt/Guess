...
public class Firstin {
    ...
    public void initialize() {
        ...
        JLabel tip=new JLabel("请输入ID（10位数字）， 密码（0-15位）");
        tip.setFont(new Font("黑体",Font.PLAIN,13));
        tip.setBounds(90,385,250,50);
        contentPanel.add(tip);
        login=new JButton("登录");
        firstin=new JButton("注册");
        ending=new JButton("退出");
        firstin.setBounds(85,420,75,50);
        firstin.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                if (N.length() == 10  && N.matches("^[0-9]+(.[0-9]+)?$")&&P.length() > 0 && P.length() <= 15) {
                    ...
                    try {
                        if (new FirstinDao(cnew).userinsql()) {
                            JOptionPane.showMessageDialog(null, "注册成功！");
                            new Login();
                            frame.setVisible(false);
                        } else {
                            JOptionPane.showMessageDialog(null, "此id已被使用！");
                        }
